...
fn main() {
    ...
    let pulse_config = PulstarConfig::read_from_toml(path);
    let ttime_points = pulse_config.get_time_points(); 
    ...
    for (n,time_stamp) in ttime_points.iter().enumerate(){
        ...
        for mode in pulse_config.mode_data.iter(){
        ...
        while  theta < 180.0 {
         ...
         while phi <= 360.0{
                ...
                if cos_chi > 0.0{ //If the shifted mass element is visible
                    ...
                    local_veloc = observed_pulsation_velocity(&pulse_config, theta_rad, phi_rad, &kk).unwrap();
